...
public class ProfileAuthService(SaveServer saveServer)
{
    ...
    public async Task<bool> SetPasswordAsync(MongoId profileId, string password)
    {
        ...
        profile.ExtensionData[AuthDataKey] = new ProfileAuthData
        {
    ...
    public MongoId Validate(string? username, string? password)
    {
        ...
        if (!TryGetProfileByUsername(username, out var profile) || !TryGetAuthData(profile, out var authData))
        {
            return MongoId.Empty();
        }

        var expectedHash = Encoding.UTF8.GetBytes(authData.PasswordHash);
    ...
    private static bool TryGetAuthData(SptProfile profile, out ProfileAuthData authData)
    {
        authData = default!;
        ...
        authData = value switch
        {
            ProfileAuthData data => data,
            JsonElement element => element.Deserialize<ProfileAuthData>()!,
            _ => JsonSerializer.Deserialize<ProfileAuthData>(JsonSerializer.Serialize(value))!,
        };

        return !string.IsNullOrWhiteSpace(authData?.PasswordHash);
    }
    ...
    private sealed record ProfileAuthData
    {
